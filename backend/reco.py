...
sp = spotipy.Spotify(client_credentials_manager=client_credentials_manager, language = "en")
...
# Fetches user playlists given spotify username
...
# Generates user track metadata dataframe given playlists_ids
def generate_user_track_metadata_df(playlist_ids):
    ...
    for idx, playlist_id in enumerate(playlist_ids):
      ...
      for item in playlist['tracks']['items']:
          track = item['track']
          if track['id'] is None: continue # if song is no longer on spotify
          ...
          try:
            songs_artist.append(track['album']['artists'][0]['name']) # song artist
          except:
            songs_artist.append("Artist Not Found") # for remixes missing artists

          # artist genre
          if track['album']['artists'][0]["id"] in artist_genres_lookup:
            artist_genres.append(artist_genres_lookup[track['album']['artists'][0]["id"]])
          else:
            artist_genres_lookup[track['album']['artists'][0]["id"]] = sp.artist(track['album']['artists'][0]["id"])["genres"]
            artist_genres.append(artist_genres_lookup[track['album']['artists'][0]["id"]])

          songs_length.append(track['duration_ms']) # song length (ms)
          songs_popularity.append(track['popularity']) # song popularity
          release_dates.append(track['album']['release_date']) # song album release date
...
def generate_user_track_audio_features_df(songs_ids):
  ...
  for increment in range(0, len(songs_ids), 100):
    if p2 >= ids_count: p2 = ids_count
    songs_features = sp.audio_features(tracks=songs_ids[p1:p2]) #max limit: 100
...
def generate_user_dfs(df_meta, df_features):
   # Concatenating metadata and audio features
   user_all_df = pd.concat([df_meta, df_features], axis=1)
   print("User_All_Df")
   print(user_all_df)

   # Dropping Duplicate Songs in Various Playlists
   print(f"Number of User Songs before Dropping Duplicates: {len(user_all_df)}")
   user_all_df = user_all_df.drop_duplicates(subset=['song_name'])
   print(f"Number of User Songs after Dropping Duplicates: {len(user_all_df)}")
...
# Generates a genre-based and release-time-based song recommendations dataframe and recoemmendation dates dataframe given user songs dataframe
def generate_recommendations_dfs(user_all_df):
   most_listened_genres = user_all_df['genres'].explode().value_counts().head(8).index.tolist()
   date_df = user_all_df["release_dates"].str.split("-", expand = True)
   date_df.columns = ["year", "month", "day"]
   reco_song_names = []

   for genre in most_listened_genres:
     typical_time_period_by_genre = date_df['year'][user_all_df[user_all_df == genre].index].value_counts().head(4).index.tolist()
     for time_period in typical_time_period_by_genre:
       possible_reco_songs = sp.search(q='genre:' + genre + time_period, type='track', limit=50, market = "US") # max = 50
       for track in possible_reco_songs['tracks']['items']:
         if track["name"] in user_all_df['song_name'].values: continue
   ...
   for track_name in reco_song_names:
     ...
     if reco_search:
       ...
       if top_track is None: continue
       reco_song_ids.append(top_track['id']) # reco spotify song id
       reco_songs_len.append(top_track['duration_ms']) # reco song length (ms)
       reco_songs_pop.append(top_track['popularity']) # reco song popularity
       reco_songs_release_dates.append(top_track['album']['release_date']) # reco song album release date
       reco_songs_albums.append(top_track['album']['name']) # song album
       try:
           reco_songs_artists.append(top_track['album']['artists'][0]['name']) # reco song artist
       except:
           reco_songs_artists.append("Artist Not Found") # for reco remixes missing artists
       # reco artist genre
       if top_track['album']['artists'][0]["id"] in reco_artist_genres_lookup:
           reco_artist_genres.append(reco_artist_genres_lookup[top_track['album']['artists'][0]["id"]])
       else:
         reco_artist_genres_lookup[top_track['album']['artists'][0]["id"]] = sp.artist(top_track['album']['artists'][0]["id"])["genres"]
         reco_artist_genres.append(reco_artist_genres_lookup[top_track['album']['artists'][0]["id"]])
   ...
   for increment in range(0, len(reco_song_ids), 100):
     if p2 >= ids_count: p2 = ids_count
     reco_songs_features = sp.audio_features(tracks=reco_song_ids[p1:p2]) #max limit: 100
   ...
   print(f"Number of Reco Songs before Dropping Duplicates: {len(reco_all_df)}")
   reco_all_df = reco_all_df.drop_duplicates(subset=['spotify_id'])
   print(f"Number of Reco Songs after Dropping Duplicates: {len(reco_all_df)}")
...
# Combined feature engineering to output uniform metrics
def feature_engineering(user_all_df, reco_all_df, user_date_df, reco_date_df):
   ...
   all_date_df = pd.concat([user_date_df, reco_date_df], axis = 0, ignore_index=True)
   ...
   model_df = model_df.drop(columns = ["song_name",	"album",	"spotify_id",	"artist",	"release_dates", "genres", "found_location", "found_publisher"])
   ...
   all_df['genres'] = all_df['genres'].apply(lambda x: x if isinstance(x, list) else [])

   print("TFIDF")
   # Term Frequency - Inverse Document Frequency (TFIDF) for Song Genres
   tfidf = TfidfVectorizer()
   tfidf_genres =  tfidf.fit_transform(all_df['genres'].apply(lambda genres: " ".join(genres)))
   genre_df = pd.DataFrame(tfidf_genres.toarray())
   genre_df.columns = ['genre' + "_" + genre for genre in tfidf.get_feature_names_out()]
   print(genre_df)
   ...
   user_feature_set = all_feature_set.iloc[:len(user_all_df)] # id: user_all_df["spotify_id"]
   reco_feature_set = all_feature_set.iloc[len(user_all_df):] # id:  reco_all_df["spotify_id"]

   return user_feature_set, reco_feature_set

# Check SpotifyRecommender.ipynb for further analysis
# Key Observation: Explained Variance Ratio always tends to be min at max number of n_components
# Generates User Summarization Vector given User Features
...
# Generates fial song recommendations from through cosine similarity metric between summarization vector and reco feature set
def generate_final_recommendations(reco_feature_set, user_summarized_vector, reco_all_df):
   ...
   reco_feature_set["cos_sim"] = reco_feature_set.apply(lambda reco_vector: cosine_similarity(user_summarized_vector.values.reshape(1, -1), reco_vector.values.reshape(1, -1)), axis = 1)
   
   reco_all_df = reco_all_df.reset_index()
   reco_all_df["cos_sim"] = reco_feature_set["cos_sim"].apply(lambda sim_2D: sim_2D[0][0]).reset_index().drop(columns = "index")
   
   num_of_reccomendations = 20
   final_recommendations = reco_all_df.sort_values(by=['cos_sim'], ascending = False).head(num_of_reccomendations)
   return final_recommendations

# Creates a spotify playlist of recommendations given username
def generate_spotify_playlist(username, final_recommendations, spotify_client_id, spotify_client_secret, playlist_create_sp):
   reco_playlist = playlist_create_sp.user_playlist_create(username, "Spotifind's Playlist!", description = f"Generated from Spotifind Recommendation Engine for {username}. Recommendations are based on user's playlists.")
   reco_playlist_id = playlist_create_sp.user_playlists(username)['items'][0]['id']
   added_songs = playlist_create_sp.user_playlist_add_tracks(username, reco_playlist_id, tracks = final_recommendations["spotify_id"].values.tolist())
   return playlist_create_sp.user_playlists(username)['items'][0]['external_urls']['spotify']

# Recomendation Engine Driver Connecting All Functions
def recommendation_driver(playlist_ids, sp):
   ...
   user_feature_set, reco_feature_set = feature_engineering(user_all_df, reco_all_df, user_date_df, reco_date_df)
   user_summarized_vector = generate_user_summarization_vector(user_feature_set)
   final_recommendations = generate_final_recommendations(reco_feature_set, user_summarized_vector, reco_all_df)
